...
/**
 * \defgroup filesystemcapifuncs C API onto secure file system.
 * The secure file system is part of the Good Dynamics Secure Storage
 * feature. This API provides a subset of C type file accessors distinguished
 * by the inclusion of a GD_ prefix in front of the name.
 *
 * Note that currently the file is not locked while open
 *
 ...
 *
 * \param args A value identifying a variable arguments list initialized with va_start. va_list is a special type defined in <cstdarg>.
 *
 * \returns <TT>int</TT> On success, the total number of characters written is returned.
 * On error, the function returns EOF and sets the error indicator (ferror).
 */
